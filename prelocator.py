...
rough_peak_threshold = 0.9              # Between 0 and 1

# Image subdivision:
# ******************************************************************
local_image_size = None # mm            Fixed value or if None split in the middle of two rough points.

# Fine intersection finding parameters:
# ******************************************************************
fine_peak_threshold = 0.2               # Between 0 and 1
fine_summation_field_width = 400 # mm   Width over which the pixels are summed up at the edges of the local image. Approximate recommendation: 1/8 to 1/4 of local image size
...
preseq = imgprc.sequence([
    ...
    imgprc.binarization(threshold=[40, 60, 60], inverted=[True, True, True], mode=mode)
    ])
...
gridfndr = imgprc.rough_grid_finding(line_thickness=70, threshold=0.8)
...
refine = imgprc.vector_intersection(threshold=0.6, field_size=800, line_thickness=70)
